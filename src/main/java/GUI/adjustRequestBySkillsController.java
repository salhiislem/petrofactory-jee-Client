...
public class adjustRequestBySkillsController implements Initializable {
	...
	public Pane AddPane(List<JobRequest> ThreeProduct) {
		...
		for (JobRequest p3 : ThreeProduct) {
			if (k == 1) {
				...
				Text prixt = new Text("approved skills: ");
				Text prix = new Text(String.valueOf(p3.getNbOfapprouvedSkills()) );
				nomt.setLayoutX(50);
				nomt.setLayoutY(160);
				nom.setLayoutX(120);
				nom.setLayoutY(145);
    ...
    public void getShowPane()
    {
		...
		{
			int nb=SD.numberSkillsApproved(jr);
		jr.setNbOfapprouvedSkills(nb);
		JD.updateJob(jr);
		System.out.println(jr.getNbOfapprouvedSkills());
		}
        JD.adjustJobRequestBYOffer(o);
     //  List<JobRequest> l =JD.findALL();
        List<JobRequest> l=JD.SortBySkill();
            for (JobRequest p:l)
        {
            AllProducts.add(p);
        }
